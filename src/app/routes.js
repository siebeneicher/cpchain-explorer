...
.get('/api/v1/rnodes/timeline/:unit/:times/:ts_start/:addr', cache.route(), async function (req, res) {
	res.setHeader('X-Used-Frontend-Cache', 'no');
	res.json(await rnodes.timeline.get(req.params.unit, parseInt(req.params.times), req.params.ts_start, req.params.addr == "ALL" ? null : req.params.addr, {fieldOnly: req.query.fieldOnly}, !!parseInt(req.query.forceUpdate)));
})
